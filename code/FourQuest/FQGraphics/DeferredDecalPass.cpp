	...
	void DeferredDecalPass::Render()
	{
		...
		for (std::set<IDecalObject*> decalObjects : decalObjectLayerRef)
		{
			for (IDecalObject* decalObjectInterface : decalObjects)
			{
				...
				decalObjectCB.TexTransform = (DirectX::SimpleMath::Matrix::CreateScale(decalInfo.Tiling.x, decalInfo.Tiling.y, 1)
					* DirectX::SimpleMath::Matrix::CreateRotationZ(decalInfo.Rotation)
					* DirectX::SimpleMath::Matrix::CreateTranslation(decalInfo.Offset.x, decalInfo.Offset.y, 0)).Transpose();
